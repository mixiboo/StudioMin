...
function parseColor(value: string | null): number {
    ...
    if (!/^[0-9a-fA-F]{6}$/.test(normalized)) throw new Error('invalid color');
...
export const data = new SlashCommandBuilder()
    ...
    .addChannelOption(option => option.setName('channel').setNameLocalizations({ ko: '채널' }).setDescription('임베드를 보낼 채널').addChannelTypes(ChannelType.GuildText).setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction) {
    if (!interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) return interaction.reply({ content: '관리자만 사용할 수 있는 명령어입니다.', ephemeral: true });
    ...
    if (!(channel instanceof TextChannel)) return interaction.reply({ content: '텍스트 채널만 선택할 수 있습니다.', ephemeral: true });
    ...
    try { parsedColor = parseColor(color); }
    catch { return interaction.reply({ content: '❌ 색상은 `#5865F2` 같은 6자리 HEX 형식으로 입력해주세요.', ephemeral: true }); }

    const embed = new EmbedBuilder().setTitle(title).setDescription(description).setColor(parsedColor);
    ...
    try {
        ...
        return interaction.reply({ content: '❌ 임베드를 전송하지 못했습니다. 봇의 채널 권한과 이미지 URL을 확인해주세요.', ephemeral: true });
    }
}
